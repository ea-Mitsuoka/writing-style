...
**どこまで見るか**: スコア 3 以上を全件、それ以下は見ない。2026-09-19 の実施では候補 169 件の
うちスコア 3 以上が 81 件で、そこから採用 5 件と既存ルールへの例追加 2 件が出た。スコア 1〜2 の
88 件は目視した範囲では作業指示が大半だった。判断材料はこの 1 回分しかないため、次の実施でも
同じ比率なら基準として確定する。
