...
> 各候補を、発話と直前の assistant 本文の両方を読んだうえで「指摘」「指摘でない」
> 「判断できない」に分け、理由を 1 行で表にして。発話だけで判定しないで。
...
**どこまで見るか**: 全件。スコアの低い候補も飛ばさない。2026-09-20 の実施では、スコア 3 以上の
81 件から 10 件、スコア 1〜2 の 92 件から 5 件の指摘が出た。下位の密度は低いが件数は無視できず、
スコアは 3 点と 1 点に 86 % が集中していて下位では順位付けとして働いていない。

件数を抑えるのは `SINCE=`（手順 1）で行う。前回の実施日以降に絞れば、月 1 回なら 1 回あたりの
件数は小さい。全期間の棚卸しは 2026-09-20 に済んでいる。

**発話だけで判定しない**: 2026-09-20 の実施で、人間の発話だけを読んで 9 件を指摘と判定したが、
直前の assistant 本文を読み直すと 5 件だった。残り 4 件は作業指示や質問で、発話の文面だけでは
区別が付かなかった。
...
不採用にした下書きが他の下書きから `[[…]]` で参照されていないか確かめ、参照していれば外す。
残すと Vault の `validate_vault.py` が wikilink 切れとして手順 5 で止める。
